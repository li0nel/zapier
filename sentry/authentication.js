...
const test = (z, bundle) =>
  z.request({ url: 'https://sentry.io/api/0/projects/' });
...
const handleBadResponses = (response, z, bundle) => {
  if (response.status === 401) {
    throw new z.errors.Error(
      ...
      'The access_token you supplied is incorrect',
      'AuthenticationError',
      response.status
    );
  }
...
module.exports = {
  config: {
    ...
    fields: [{ key: 'access_token', label: 'Access Token', required: true }],
    ...
    connectionLabel: (z, bundle) => bundle.authData.access_token.substr(0, 5) + '*****************',
  },
